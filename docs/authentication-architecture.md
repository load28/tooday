...
| 내용 | `userId(sub)` + `exp`만 | 없음(키일 뿐) |
| 검증 | 서명 확인만 | 저장소 조회 |
| 수명 | 짧다 (기본 15분) | idle 14일(슬라이딩) / absolute 90일(하드캡) |
| 쓰임 | 매 요청 인증 | 액세스 만료 시 재발급 |

- **핫패스가 저장소를 안 탄다** — 매 요청은 액세스 JWT 서명 검증만 한다(`AccessTokenService`).
  세션 조회가 사라져 Redis/DB 부하가 인증에서 빠진다. 유저 프로필은 JWT에 담지 않고
  `user.me`에서만 지연 조회한다(프로필 변경이 토큰 만료까지 지연되지 않게).
...
- 같은 계보(`family_id`)를 공유한다. **이미 회전된(superseded) 토큰이 다시 제시되면 탈취 신호**로 보고 계보 전체를 무효화한다 — 탈취범이 회전시킨 뒤 정당 사용자(또는 그 반대)가 옛 토큰을 쓰는 순간 세션 전체가 죽어 피해가 제한된다.

### 무상태 액세스의 트레이드오프

액세스 JWT는 저장소를 안 타므로 **즉시 무효화가 불가**하다. 로그아웃/재사용 탐지는
리프레시(회전 경로)를 죽이지만, 이미 발급된 액세스는 만료(15분)까지 유효하다. 짧은
수명이 이 창을 좁힌다 — 즉시 무효화가 필요하면 액세스 TTL을 더 줄이거나 블랙리스트를 얹는다.
...
- tRPC 컨텍스트가 요청마다 액세스 JWT를 검증해 `ctx.userId`를 채운다 — 저장소를 타지 않는다
