...
- **apps/push-server** — BFF와 독립인 Rust(tokio) 푸시 서버. 태스크 시작 시간이
  되면 구독 기기(`push_subscriptions`)로 푸시를 보낸다 — 폴링 + `task_push_sends`
  dedup claim으로 다중 인스턴스에서도 1회 발송. DB는 공유하되 스키마 소유는 분리:
  푸시 테이블은 자체 마이그레이션으로 직접 관리하고 BFF 테이블은 읽기만 한다.
  구독 등록/해제 HTTP API 내장(BFF 액세스 JWT를 시크릿 공유로 검증, port 3003).
  발송기는 플러그블(log/expo). [apps/push-server/README.md](apps/push-server/README.md) 참고.
