...
/*
 * 인메모리 인증 세계의 행동 명세 — 실제 구현(러스트 API apps/api/src/auth/*)과 의미가
 * 일치해야 app.test.ts의 조립 검증이 유효하다. 실제 구현의 계약은 러스트 단위 테스트와
 * E2E가 검증한다.
 */
...
function make({ idleTtlMs, absoluteTtlMs }: { idleTtlMs: number; absoluteTtlMs: number }) {
  return {
...
describe('인메모리 인증 스토어 명세', () => {
  it('유저를 생성한다 (이메일은 정규화)', async () => {
    const { users } = make(LONG);
  ...
  it('리프레시를 발급하고, 회전은 새 토큰을 주며 옛 토큰을 폐기한다', async () => {
    const { users, refreshTokens } = make(LONG);
  ...
  it('isSessionLive: absolute 만료 세션은 dead', async () => {
    const { users, refreshTokens } = make({ idleTtlMs: 60_000, absoluteTtlMs: -1 });
    const user = await users.create(INPUT);
    const issued = await refreshTokens.issue(user.id);
    expect(await refreshTokens.isSessionLive(issued.sessionId)).toBe(false);
  });
  ...
  it('회전은 idle을 슬라이딩하되 absolute 하드캡을 넘지 않는다', async () => {
    const { users, refreshTokens } = make({ idleTtlMs: 60_000, absoluteTtlMs: 30_000 });
    const user = await users.create(INPUT);
    const issued = await refreshTokens.issue(user.id);
  ...
  it('idle 만료 토큰은 회전되지 않고, deleteExpired가 청소한다', async () => {
    const { users, refreshTokens } = make({ idleTtlMs: -1, absoluteTtlMs: 120_000 });
    const user = await users.create(INPUT);
