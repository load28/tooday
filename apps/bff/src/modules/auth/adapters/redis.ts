...
export class RedisRefreshTokenStore implements RefreshTokenStore {
  ...
  async issue(userId: string): Promise<RefreshToken> {
    const now = Date.now();
    return this.write({ userId, sessionId: newId(), absoluteExpiresAt: now + this.absoluteTtlMs, now });
  }
  ...
  async rotate(token: string): Promise<RefreshToken | null> {
    ...
    return this.write({ userId: stored.userId, sessionId: stored.sessionId, absoluteExpiresAt: stored.absoluteExpiresAt, now });
  }

  async revoke(token: string): Promise<void> {
    const raw = await this.redis.get(this.tokenKey(token));
    if (!raw) return;
    const { sessionId } = JSON.parse(raw) as StoredToken;
  ...
  async isSessionLive(sessionId: string): Promise<boolean> {
    ...
    return this.redis.exists(this.sessionKey(sessionId));
  }
  ...
  private async revokeSession(sessionId: string): Promise<void> {
    const hashes = await this.redis.smembers(this.sessionKey(sessionId));
    const keys = hashes.map((hash) => `${TOKEN_PREFIX}${hash}`);
    if (keys.length > 0) await this.redis.del(...keys);
    await this.redis.del(this.sessionKey(sessionId));
  }

  /** 새 토큰을 발급해 idle TTL(absolute 캡)로 저장하고 세션 SET에 등록한다 — issue와 rotate의 공통 경로. */
  private async write({
    userId,
    sessionId,
    absoluteExpiresAt,
    ...
    const value: StoredToken = { userId, sessionId, absoluteExpiresAt, superseded: false };
    ...
    await this.redis.sadd(this.sessionKey(sessionId), hash);
    await this.redis.pexpire(this.sessionKey(sessionId), Math.max(1, absoluteExpiresAt - now));
    return { token, userId, sessionId, expiresAt: now + pxMs, absoluteExpiresAt };
  }
}
