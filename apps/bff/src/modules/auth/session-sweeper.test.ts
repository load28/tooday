...
/** deleteExpired만 의미 있게 채운 SessionStore 스텁 — 스윕은 이 포트 하나만 쓴다. */
function fakeSessions(deleteExpired: SessionStore['deleteExpired']): SessionStore {
  return {
    create: async () => {
      throw new Error('unused');
    },
    get: async () => null,
    getWithUser: async () => null,
    revoke: async () => {},
    deleteExpired,
  };
}
...
describe('startSessionSweep', () => {
  ...
  it('부팅 시 즉시 1회 청소하고, 지운 게 있으면 로그를 남긴다', async () => {
    ...
    let count = 0;
    const stop = await startSessionSweep({
    ...
    stop();
    expect(count).toBe(1);
    expect(calls).toContainEqual({ level: 'info', message: 'sessions_swept', fields: { deleted: 3 } });
  });

  it('지운 게 없으면 로그를 남기지 않는다', async () => {
    const { logger, calls } = fakeLogger();
    const stop = await startSessionSweep({ sessions: fakeSessions(async () => 0), logger, intervalMs: INTERVAL });
    stop();
  ...
  it('deleteExpired가 던져도 삼키고 error 로그를 남긴다', async () => {
    ...
    stop();
    expect(calls).toContainEqual({ level: 'error', message: 'session_sweep_failed', fields: { message: 'boom' } });
  });
});
