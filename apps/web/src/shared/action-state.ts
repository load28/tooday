...
type ActionState = { pendingCount: number; error: unknown };

/** 결과 데이터는 DB에 둔다. 이 Store는 컴포넌트 트리의 실행 상태만 소유한다. */
export function createActionState() {
  const state = createAtom<ActionState>({ pendingCount: 0, error: null });
  let latest = 0;
...
export function useActionState() {
  const [execution] = useState(createActionState);
