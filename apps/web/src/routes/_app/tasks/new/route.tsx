...
import { NewTaskSheetStoreProvider } from '@/features/tasks/new-task-sheet-store';

export const Route = createFileRoute('/_app/tasks/new')({
  // 프로젝트 선택지를 미리 채워 첫 렌더에서 suspend 하지 않는다. now는 기본 날짜(오늘)를 고정한다.
  loader: async ({ context }) => {
    await context.taskCacheSession.get(context.user.id).preload({ kind: 'projects' });
    return { now: Date.now() };
  },
...
function NewTaskRoute() {
  ...
  return (
    <NewTaskSheetStoreProvider>
      <NewTaskScreen now={now} renderNewProjectSheet={(props) => <NewProjectSheet {...props} />} />
    </NewTaskSheetStoreProvider>
  );
}
