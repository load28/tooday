...
export const cardCls = style(util({ display: 'flex', alignItems: 'flex-start', gap: vars.space.xl }));
...
export const checkRecipe = recipe({
  ...
  variants: {
    status: {
      todo: util({}),
      doing: util({ background: vars.color.primarySoft, borderColor: vars.color.primary, color: vars.color.primary }),
      done: util({ background: vars.color.success, borderColor: vars.color.success, color: vars.color.textInverse }),
    },
  },
});
