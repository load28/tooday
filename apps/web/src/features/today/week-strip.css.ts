...
export const cellRecipe = recipe({
  ...
  variants: {
    tone: {
      idle: util({ color: vars.color.textTertiary }),
      today: util({ color: vars.color.primary }),
    },
  },
});

export const dowCls = style(util({ ...textStyles.micro, marginBottom: vars.space.xs }));
export const dayCls = style(util({ ...textStyles.numericLg }));
...
export const dotRecipe = recipe({
  ...
  variants: {
    mark: {
      none: util({ background: 'transparent' }),
      tasks: util({
